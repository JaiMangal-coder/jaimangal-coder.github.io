...
I went straight into the csv, stacked the two columns and deleted the extra column names (eg. primary, thp1). This made my life easier.

```{r}
#| size: "scriptsize"
#| 
dat = read.csv(file = "Fig. 3B-Table 1.csv")
dat = dat[,c(1,2)] #just want the first two columns
...
I used a bunch of mutate-case_when's. I kept using pipes to transfer the data over oafter each mutate was run so that it way it changes the data in the file all at the same time. 
```{r}
#| size: "scriptsize"
dat.kitsberg.cleaned = dat.kitsberg.cleaned |>
...
#| size: "scriptsize"
#| fig-align: "center"
#| fig-cap: "HCMV Genomes in Monocytes vs Macrophages"
...
dat.kitsberg.cleaned |>
  ...
  summarize(
    ...
    Max = max(viruses.within.nucleus),
...
This tells us that there were less viruses within monocyte cells, as compared to macrophage cells. 
...
dat = read.csv(file = "ShipValuation-Bal25.csv")
...
```{r}
#| size: "scriptsize"
#| fig-cap: "Ship Prices by Country of Origin"


ggplot(dat.bal.cleaned |> drop_na(OriginCountry))+
  geom_boxplot(aes(x=OriginCountry, y = price), 
                  fill="lightgrey")+
  theme_bw()+
  labs(x="Country of Origin", y="Ship Price (in Millions of US Dollars)")
```
...
```{r}
#| size: "scriptsize"
#| fig-cap: "Ship Price vs Ship Age"


ggplot(dat.bal.cleaned, aes(x = Yas, y = price)) +
  geom_point(color = "black") +
  
  theme_bw() +
  labs(x = "Ship Age (Years)",
       y = "Ship Price (in Millions of US Dollars)")
```
...
Here, we obviously see an inverse relationship between ship age and age. As ships get older, their price gradually decreases. There are a few outliers, but this can most likely be attributed to specific ship features and/or specialties which boost prices. 